...
export default function PetsList({ onEdit, pets }) {
    const [deletePet, { data: mutationData, error: mutationError }] = useMutation(DeletePet, {
        update: (
            cache,
            {
                data: {
                    deletePet: { id: deletedPetId },
                },
            }
        ) => {
    ...
    return (
        <div className="row">
            {pets.map((pet) => (
                ...
                >
                    <div className="box">
                        <PetBox onDelete={onDelete} onEdit={onEdit} pet={pet} />
                    </div>
                </div>
            ))}
